...
//=============================================================================
gpu::Fence::Fence(Fence&& old) noexcept : m_sync(std::exchange(old.m_sync, nullptr)) {}
//=============================================================================
gpu::Fence& gpu::Fence::operator=(Fence&& old) noexcept
{
	if (this == &old)
		return *this;
	this->~Fence();
...
//=============================================================================
void gpu::Fence::Signal()
{
	assert(m_sync == nullptr);
	m_sync = glFenceSync(GL_SYNC_GPU_COMMANDS_COMPLETE, 0);
}
//=============================================================================
uint64_t gpu::Fence::Wait()
{
	assert(m_sync != nullptr);
	GLuint id;
	glGenQueries(1, &id);
